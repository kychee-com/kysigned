...
function skillList(origin: string): Skill[] {
  ...
  const verify: Skill = {
    ...
    body: [
      ...
      '- `--offline` makes no network request. The Bitcoin timestamp anchor then reports pending, and the key archive',
      "  reports pending unless the bundle carries the archive's signed statement, which confirms the key offline. The",
      '  verdict still holds.',
      '',
