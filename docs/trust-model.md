...
- **Leaked or rotated-published DKIM key (the `dkim-rotate` model).** A provider's old private key leaks, or
  ...
  **anchored-time upper bound** on the key's life. The public archive records when it
  ...
  that is the same residual risk as any PKI.
...
- **Replay / double-recording.** The same signed email is submitted twice, or two
  workers race to record it. *Defence:* recording is idempotent, so a still-pending
...
- **Mailbox compromise.** An attacker controls the signer's actual email account.
  *Defence:* **out of scope**, because no e-signature product defends against a fully
